...
const rows: Row[] = [
  ...
  {
    name: 'inlining a reference to a boolean schema',
    schema: { $defs: { yes: true }, type: 'object', properties: { p: { $ref: '#/$defs/yes' } } },
    profile: noRefs,
    fitted: { type: 'object', properties: { p: true } },
    changes: [
      { path: '/properties/p/$ref', rule: 'no-ref', narrowing: false },
      { path: '/$defs', rule: 'no-defs', narrowing: false },
    ],
  ...
  {
    name: 'inlining a reference that points at another reference',
    schema: {
      $defs: { outer: { $ref: '#/$defs/inner' }, inner: { type: 'string' } },
      type: 'object',
    ...
    fitted: { type: 'object', properties: { p: { type: 'string' } } },
    changes: [
      { path: '/$defs/outer/$ref', rule: 'no-ref', narrowing: false },
      { path: '/$defs/outer/$ref', rule: 'no-ref', narrowing: false },
      { path: '/properties/p/$ref', rule: 'no-ref', narrowing: false },
      { path: '/$defs', rule: 'no-defs', narrowing: false },
    ],
  ...
  {
    name: 'the legacy definitions section is removed too',
    schema: { definitions: { a: { type: 'string' } }, type: 'object', properties: { p: { $ref: '#/definitions/a' } } },
    profile: noRefs,
    fitted: { type: 'object', properties: { p: { type: 'string' } } },
    changes: [
      { path: '/properties/p/$ref', rule: 'no-ref', narrowing: false },
      { path: '/definitions', rule: 'no-defs', narrowing: false },
    ],
  },
  {
    name: 'a reference whose neighbours cannot be folded in is kept beside them when allOf is available',
    schema: { $defs: { a: { pattern: '^a' } }, properties: { p: { $ref: '#/$defs/a', pattern: '^b' } } },
    profile: noSiblings,
    fitted: {
      $defs: { a: { pattern: '^a' } },
      properties: { p: { allOf: [{ pattern: '^a' }, { pattern: '^b' }] } },
    },
    changes: [{ path: '/properties/p/$ref', rule: 'ref-siblings', narrowing: false }],
  },
  {
    name: 'a reference whose neighbours cannot be folded in accepts nothing without allOf',
    schema: { $defs: { a: { pattern: '^a' } }, properties: { p: { $ref: '#/$defs/a', pattern: '^b' } } },
    profile: variant(noSiblings, { supports: { allOf: false } }),
    fitted: { $defs: { a: { pattern: '^a' } }, properties: { p: NOTHING } },
    changes: [{ path: '/properties/p/$ref', rule: 'ref-siblings', narrowing: true }],
  },
  {
    name: 'patternProperties on an already closed object loses nothing',
    schema: {
      type: 'object',
    ...
    changes: [{ path: '/patternProperties', rule: 'no-pattern-properties', narrowing: false }],
  },
  {
    name: 'a pattern matching none of the declared properties just closes the object',
    schema: { type: 'object', properties: { zz: { type: 'string' } }, patternProperties: { '^a': { minLength: 2 } } },
    profile: variant(permissive, { supports: { patternProperties: false } }),
    fitted: { type: 'object', properties: { zz: { type: 'string' } }, additionalProperties: false },
    changes: [{ path: '/patternProperties', rule: 'no-pattern-properties', narrowing: true }],
  },
  {
    name: 'patternProperties with nothing declared closes the object to everything',
    schema: { type: 'object', patternProperties: { '^a': true } },
    profile: variant(permissive, { supports: { patternProperties: false } }),
    fitted: { type: 'object', additionalProperties: false },
    changes: [{ path: '/patternProperties', rule: 'no-pattern-properties', narrowing: true }],
  },
  {
    name: 'an empty list of per-position item schemas is just removed',
    schema: { type: 'array', prefixItems: [] },
    profile: gemini,
    fitted: { type: 'array' },
    changes: [{ path: '/prefixItems', rule: 'no-tuple-items', narrowing: false }],
  },
  {
    name: 'per-position schemas that constrain nothing leave no item schema behind',
    schema: { type: 'array', prefixItems: [true, true] },
    profile: gemini,
    fitted: { type: 'array' },
    changes: [{ path: '/prefixItems', rule: 'no-tuple-items', narrowing: true }],
  },
  {
    name: 'the rest schema is folded into the single item schema as well',
    schema: { type: 'array', prefixItems: [{ type: 'string' }], items: { minLength: 2 } },
    profile: gemini,
    fitted: { type: 'array', items: { type: 'string', minLength: 2 } },
    changes: [{ path: '/prefixItems', rule: 'no-tuple-items', narrowing: true }],
  },
  {
    name: 'the draft-07 tuple spelling collapses the same way',
    schema: { type: 'array', items: [{ type: 'string', minLength: 1 }, { type: 'string' }] },
    profile: gemini,
    fitted: { type: 'array', items: { type: 'string', minLength: 1 } },
    changes: [{ path: '/items', rule: 'no-tuple-items', narrowing: true }],
  },
  {
    name: 'a list holding only null keeps null',
    schema: { type: ['null'] },
    profile: gemini,
    fitted: { type: 'null' },
  ...
  {
    name: 'a list of types with nothing in it accepts nothing',
    schema: { type: [] },
    profile: gemini,
    fitted: NOTHING,
    changes: [{ path: '/type', rule: 'no-nullable-via-type', narrowing: false }],
  ...
  {
    name: 'an empty anyOf accepts nothing',
    schema: { anyOf: [] },
    profile: variant(permissive, { supports: { anyOf: false } }),
    fitted: NOTHING,
    changes: [{ path: '/anyOf', rule: 'no-anyof', narrowing: false }],
  },
  {
    name: 'an empty oneOf accepts nothing',
    schema: { oneOf: [] },
    profile: variant(permissive, { supports: { oneOf: false } }),
    fitted: NOTHING,
    changes: [{ path: '/oneOf', rule: 'no-oneof', narrowing: false }],
  ...
  {
    name: 'a single anyOf option folds in without loss',
  ...
  {
    name: 'a single anyOf option that cannot be folded in accepts nothing',
    schema: { pattern: '^a', anyOf: [{ pattern: '^b' }] },
    profile: variant(permissive, { supports: { anyOf: false, allOf: false } }),
    fitted: NOTHING,
    changes: [{ path: '/anyOf', rule: 'no-anyof', narrowing: true }],
  },
  {
    name: 'a root already listing only object is written out plainly',
    schema: { type: ['object'], properties: { a: true } },
    profile: variant(permissive, { rootMustBeObject: true }),
    fitted: { type: 'object', properties: { a: true } },
    changes: [{ path: '', rule: 'root-must-be-object', narrowing: false }],
  },
  {
    name: 'an object at the property limit is left alone',
    schema: { type: 'object', properties: { a: true, b: true } },
    profile: narrow,
    fitted: { type: 'object', properties: { a: true, b: true } },
    changes: [],
  },
  {
    name: 'a limit of none leaves nothing but a closed object',
    schema: { type: 'object', properties: { a: true } },
    profile: variant(permissive, { maxProperties: 0 }),
    fitted: { type: 'object', properties: {}, additionalProperties: false },
